...
def get_roster(season):
    ...
    future = [asyncio.ensure_future(get_team_selector(TEAM_LIST[i], season, '#roster')) for i in range(30)]
    roster_df = loop.run_until_complete(future)
    ...
    df = pd.concat([df, roster_df], axis=0)
    return df

def get_team_series(team, season, data_format='PER_GAME'):
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(get_team_selector(team, season, '#team_and_opponent'))
    series = loop.run_until_complete(future)
...
def get_opp_series(team, season, data_format='PER_GAME'):
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(get_team_selector(team, season, '#team_and_opponent'))
    s1 = loop.run_until_complete(future)
...
def get_roster_stats(team, season, data_format='PER_GAME', playoffs=False):
    ...
    selector = '#'+selector
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(get_team_selector(team, season, selector))
    roster_df = loop.run_until_complete(future)
...
def get_team_misc(team, season, data_format='PER_GAME'):
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(get_team_selector(team, season, '#team_misc'))
    team_misc_df = loop.run_until_complete(future)
...
def get_player_salaries(team, season):
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(get_team_selector(team, season, '#salaries2'))
    salaries_df = loop.run_until_complete(future)
